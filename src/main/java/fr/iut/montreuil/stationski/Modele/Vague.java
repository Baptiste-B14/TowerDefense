    ...
    public void prochaineVague(){
        ...
        while (this.listEnnemis.size()<10){
            if ((Math.random() * 1)<this.pourcentChanceSkieurBasique)
                this.listEnnemis.add(new SkieurBasique(100, 1, 1, 1, env, 5, this.dijkstra.getParcours())); // new Skieur

            if ((Math.random() * 1)<this.pourcentChanceSnowboardeur)
                this.listEnnemis.add(new Ennemi(100, 1, 1, 2, env, 10, this.dijkstra.getParcours())); // new Snowboardeur

            if ((Math.random() * 1)<this.pourcentChanceLuge)
                this.listEnnemis.add(new Ennemi(100, 1, 1, 2, env, 15, this.dijkstra.getParcours())); // new Luge
